<x-layout>
    ...
    <form method="POST" action="{{ route('store') }}">
        <div class="max-w-5xl mx-auto">
            @csrf
            <div class="w-full px-4 md:w-3/3 lg:w-5/5">
                <div class="mb-2">

                    <input type="text" name="title" placeholder="Default Input"
                        class="@error('title') is-invalid @enderror w-full bg-transparent rounded-md border border-stroke dark:border-dark-3 py-[10px] px-5 text-dark-6 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-2 disabled:border-gray-2" />
                    <div class="mb-2">
                        @error('title')
                            <small class="font-xs text-red-500">{{ $message }}</small>
                        @enderror
                    </div>
                </div>
            </div>
                    ...
                    <div class="relative z-20">
                        <select name="employee_id" id="employee_id"
                            ...
                            @foreach ($employees as $employee)
                                <option value="{{ $employee->id }}" class="dark:bg-dark-2">{{ $employee->name }}
                                </option>
                            @endforeach
                        </select>
                        <span
            ...
            <div class="w-full px-4 md:w-3/3 lg:w-5/5">
                <div class="mb-2">
                    ...
                    <input type="text" name="salary" placeholder="Salary"
                        class="@error('salary') is-invalid @enderror w-full bg-transparent rounded-md border border-stroke dark:border-dark-3 py-[10px] px-5 text-dark-6 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-2 disabled:border-gray-2" />
                    <div class="mb-2">
                        @error('salary')
                            <small class="font-xs text-red-500">{{ $message }}</small>
                        @enderror
            ...
            <div class="w-full px-4 md:w-3/3 lg:w-5/5">
                <div class="mb-12">
                    <div class="relative">
                        <textarea type="text" name="body_text" rows="6" placeholder="Type your message"
                            class="@error('body_text') is-invalid @enderror w-full bg-transparent rounded-md border border-stroke dark:border-dark-3 p-3 pl-12 text-dark-6 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-2"></textarea>
                        <div class="mb-2">
                            @error('body_text')
                                <small class="font-xs text-red-500">{{ $message }}</small>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="tag" placeholder="tag"
                                class="@error('tag') is-invalid @enderror w-full bg-transparent rounded-md border border-stroke dark:border-dark-3 py-[10px] px-5 text-dark-6 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-2 disabled:border-gray-2">
                            @error('tag')
                                <small class="font-xs text-red-500">{{ $message }}</small>
                            @enderror
                        </div>
                    ...
                    <button type="submit"
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Create Job</button>
                </div>
            </div>
        </div>
